...
full_model <- svyglm(NCHILD ~ LIT_HEAD + INCEARN_HEAD + URBAN + PERSONS, design = svy_data)

summary(full_model)


```
When we control for literacy of Heads of Household, Income for heads of household,number of people in each household and urban-rural status, we find a strong relationship. Holding all other areas constant, an addition of one person per household is associated with in increase of about 0.5 children per household. In other terms, a child is added for every two persons per household. This relationship is significant at a 95% confidence level.

On the other hand, an additional income of 10,000,000 Mexican pesos by heads of household is associated with an increase of 1.791 children. or an increade in income of 11,173,184.4 pesos is associated with an increase of 2 children in the household. This relationship is significant at a 95% confidence level.

Due to high levels of literacy in our data sample. When we control for everything else, Literate heads of households have about 0.1576 children more than non-literate heads of household.This relationship is significant at a 95% confidence level.

Urban/rural settlements was also a significant predictor of number of children. When we control for everything else, Urban households have about 0.1020 children fewer than rural households. This relationship is significant at a 95% confidence level.


```{r bivariate plot income vs pop change, message=FALSE, warning=FALSE} 

income_children_plot <- ggplot(data = data, aes(INCEARN_HEAD, NCHILD))+
  geom_point() + ylim(0, 15)+
  facet_wrap(vars(GEO1_MX))
  
  
income_children_plot + stat_smooth(
              method = "lm",
              formula = y ~ x,
              geom = "smooth")
```
...
centered_data <- svy_data %>%
  mutate(INCEARN_HEAD = INCEARN_HEAD - earnedincome_per_head_mean$mean,
         NCHILD = NCHILD  - children_per_household_mean$mean)
...
centered_model <- svyglm(NCHILD ~ LIT_HEAD + INCEARN_HEAD + URBAN + PERSONS, svy_centered)
...
centered_data_scaled <- centered_data %>%
  mutate(INCEARN_HEAD_10k = INCEARN_HEAD / 10000)
...
coeff_names_scaled <- c("Constant" = "(Intercept)",
                 "Income" = "INCEARN_HEAD",
                  "Income ($10,000s)" = "INCEARN_HEAD_10k",
                 "Literacy" = "LIT_HEADYes, literate",
                 "Rural-Urban" = "URBANUrban",
                 "People in Household" = "PERSONS")
                   
centered_model_scaled <- svyglm(NCHILD ~ LIT_HEAD + INCEARN_HEAD_10k + URBAN + PERSONS, svy_centered_scaled)
...
data_scaled_log <- svy_data %>%
  mutate(log2_INCEARN_HEAD = log(INCEARN_HEAD + earnedincome_per_head_mean$mean, base = 2))


svy_scaled_log <- data_scaled_log %>%
  as_survey_design(ids = CLUSTER,
                   strata = STRATA,
                   weights = PERWT)
                   

model_scaled_log <- svyglm(NCHILD ~ LIT_HEAD + log2_INCEARN_HEAD + URBAN + PERSONS, svy_scaled_log)


coeff_names_scaled_log <- c("Constant" = "(Intercept)",
                 "Income" = "INCEARN_HEAD",
                  "Income ($10,000s)" = "INCEARN_HEAD_10k",
                 " Doubled income" = "log2_INCEARN_HEAD",
                 "Literacy" = "LIT_HEADYes, literate",
                 "Rural-Urban" = "URBANUrban",
                 "People in Household" = "PERSONS")

export_summs(full_model, centered_model, centered_model_scaled, model_scaled_log, 
             error_format = "(p = {p.value})",
             error_pos = "same",
             model.names = c("Initial", "Centered", "Centered and scaled", "Scaled,\nand logged"),
...
Our data here shows that doubling a head of houshold’s income is likely to increase the number of children by .02. This result is significant at at 99% confidence level.


## Interactions
### Estimating a new model with an interaction between Literacy and People per household

```{r message=FALSE, warning=FALSE}


interaction_model <- svyglm(NCHILD ~ LIT_HEAD + log2_INCEARN_HEAD + URBAN + PERSONS + LIT_HEAD:PERSONS, svy_scaled_log)

export_summs(model_scaled_log, interaction_model, 
             error_format = "(p = {p.value})",
             error_pos = "same",
             model.names = c("Non linear Transformation: Dobling Income", "Interaction: Literacy and People per household"))


```
#### Interpretation: 

The coefficient for the interaction term (Literacy and Number of people per household) is significant and positive. It’s the same as the coefficient for the doubled income transformation -our preferred model-, with this we can reasonably assume, with 99.9% confidence, that our dependent variable (NCHILD "Number of children per household") is likely to increase by .02 units.
